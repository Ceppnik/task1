  ...
  if (options.input) {
      ...
      if (err) {
        ...
        if (options.output) {
          fs.stat(options.output, (err) => {
            if (err) {
              err.code === "ENOENT"
                ? console.error(`Output file ${options.output} not found`)
                : console.error(err.message);
              process.exit(1);
            } else {
          ...
          readStream.on("data", (data) =>
            console.log(
              caesar(data.toString(), parseInt(options.shift), options.action)
            )
          );
        }
      }
    ...
    process.stdin.on("readable", () => {
      ...
      if (text && text == "Ctrl+C") {
        ...
        console.log(
          caesar(String(text), parseInt(options.shift), options.action)
        );
